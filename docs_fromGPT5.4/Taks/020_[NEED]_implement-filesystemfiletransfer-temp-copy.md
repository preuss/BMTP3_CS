# Task 020: implement filesystemfiletransfer temp copy

**Filnavn:** 020_[NEED]_implement-filesystemfiletransfer-temp-copy.md  
**Prioritet:** [NEED]  
**Milestone-spor:** Milestone 1: Filesystem baseline via CLI

## Formaal
Opret `Transfer/Filesystem/FilesystemFileTransfer.cs` (implementerer `IFileTransfer`). Kopiér kilde → `dest.tmp` i chunks (buffer fra `Core4Options`, default 1 MB). Atomisk rename `.tmp` → endelig sti.
