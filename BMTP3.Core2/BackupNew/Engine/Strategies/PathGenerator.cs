		...
		foreach (char c in InvalidFileNameChars)
		{
			if (fileName.Contains(c))
			{
				throw new ArgumentException(
					$"File name '{fileName}' contains invalid character: '{c}' (0x{(int)c:X2}).");
			}
		}
	...
	private string GetTokenValueStrict(string key, IBackupItem item, string originalFileName)
	{
		...
		switch (key)
		{
			...
			default:
				...
				throw new ArgumentException(
					$"Invalid template variable '${key}'. Variable names are case-sensitive (e.g., use ${{YYYY}}, not ${{yyyy}}).");
		}
	}

	private string GetHash(IBackupItem item, int length)
	{
		if (!item.Metadata.Has(MetadataKey.Hashes))
		{
			return "nohash";
		}
		...
		string hash = "";
		if (hashes.ContainsKey(HashType.BLAKE3_512))
		{
			hash = hashes[HashType.BLAKE3_512];
		}
		else if (hashes.ContainsKey(HashType.SHA2_256))
		{
			hash = hashes[HashType.SHA2_256];
		}
		else if (hashes.ContainsKey(HashType.MD5_128))
		{
			hash = hashes[HashType.MD5_128];
		}
		else
		{
			hash = hashes.Values.FirstOrDefault() ?? "nohash";
		}
