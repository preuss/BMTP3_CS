...
public sealed class JobStateMachine : IStateMachine<JobState>
{
	...
	public bool CanTransition(JobState from, JobState to) => JobTransitions.TryGetValue(from, out var allowed) && allowed.Contains(to);
	...
	public void Apply(BackupJob job, JobState to)
	{
		...
		job.State = to;
		job.Audit.Add(new AuditEntry
		{
			Stage = $"Job:{to}",
			Error = null,
			AttemptCount = (uint)(job.Audit.Count + 1),
