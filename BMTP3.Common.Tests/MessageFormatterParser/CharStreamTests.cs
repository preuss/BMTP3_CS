...
public class CharStreamTests
{
	...
	public void PeekMultiple_WithTooManyChars_ThrowsException()
	{
		...
		Assert.Equal(
			$"Not enough characters in stream (requested 3, found 2) at line {lineNumberExpected}, column {columnNumberExpected}",
			exception.Message);
		Assert.Equal(lineNumberExpected, exception.LineNumber);
		Assert.Equal(columnNumberExpected, exception.ColumnNumber);
	}
	...
	{
		// Arrange
		CharStream stream = new(input, [newLineSequence]);
	...
	[InlineData("a\n\rb", 3, 1, 1)]
	[InlineData("", 0, 0, 0)]
	public void PositionTracking_MultipleReads_UpdatesCorrectly(string input, int charCount, int expectedLine,
		int expectedColumn)
	{
		// Arrange
		string[] allNewLineSequences = new[]
			{ CharStream.POSIX, CharStream.WINDOWS_DOS, CharStream.COMMODORE, CharStream.ACORN };
