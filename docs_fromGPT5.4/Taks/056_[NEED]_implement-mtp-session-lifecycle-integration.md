# Task 056: implement mtp session lifecycle integration

**Filnavn:** 056_[NEED]_implement-mtp-session-lifecycle-integration.md  
**Prioritet:** [NEED]  
**Milestone-spor:** Milestone 2: MTP baseline end-to-end

## Formaal
MTP-session åbnes af kaldende klasse, ikke af scanner/transfer. Keepalive mindst hver `Core4Options.MtpKeepAliveInterval`.
...
```mermaid
flowchart LR
    Device[MTP device] --> Scan[MTP scanner]
    Scan --> Transfer[MTP transfer]
    Transfer --> Sidecar[Sidecar update]
    Sidecar --> Summary[Backup result]
```
## Klasse-/flow-note
