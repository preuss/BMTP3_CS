...
public class TypeChecker
{
	...
	private void ValidatePlaceholder(PlaceholderNode node)
	{
		...
		Type valueType = knownValues[node.NameOrIndex];
		...
		if (node.Condition != null)
		{
			...
			string opText = (node.Condition.ConditionOperator as TextNode)?.Value ?? string.Empty;
			if (!IsValidCondition(opText))
			{
				throw new InvalidOperationException($"Invalid condition: {opText}");
			}

			foreach (AstNode item in node.Condition.TrueValue.Concat(node.Condition.FalseValue))
			{
				if (item is PlaceholderNode nested)
				{
					ValidatePlaceholder(nested);
	...
	{
		return new[] { "number", "date", "string", "if" }.Contains(type);
	}
	...
	private bool IsTypeCompatible(string nodeType, Type valueType)
	{
		...
		if (nodeType == "date")
		{
			return valueType == typeof(string); // Antager strengbaserede datoer
		}

		if (nodeType == "if")
		{
			return true; // Betingelser kan bruges med alle typer
		}
	...
	{
		return new[] { "eq0", "gt0", "gte0", "lt0", "lte0", "ne0", "in", "nin" }.Contains(condition);
	}
}