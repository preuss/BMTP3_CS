...
internal sealed class NoopMediaDeviceScanner : ITraversalScanner<MediaFileInfo>
{
	...
	{
		_logger?.LogDebug("NoopMediaDeviceScanner.Scan called for '{RootPath}' — no MTP device registered, yielding nothing.", rootPath);
		return Enumerable.Empty<MediaFileInfo>();
	}

	public async IAsyncEnumerable<MediaFileInfo> ScanAsync(string rootPath, bool recursive = true, IProgress<TraversalProgress>? progress = null, [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
	{
		_logger?.LogDebug("NoopMediaDeviceScanner.ScanAsync called for '{RootPath}' — no MTP device registered, yielding nothing.", rootPath);
