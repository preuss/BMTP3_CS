...
public class CollisionResolverTests
{
	...
	public async Task ResolveAsync_WhenSidecarMissing_UsesHashGeneratorFallback()
	{
		...
		try
		{
			...
			CollisionResolver resolver = new(new DummyMetadataReader(), new SimplePathGenerator(),
				LoggerFactory.Create(b => { }).CreateLogger<CollisionResolver>(), new FakeHashGenerator());
	...
	public async Task ResolveAsync_Uses_DestinationHashes_From_Item_Metadata_First()
	{
		...
		try
		{
			...
			Dictionary<HashType, string> destHashes = new() { { HashType.SHA2_256, shaHex } };
	...
	private class FakeHashGenerator : IHashGenerator
	{
			...
			IEnumerable<HashType> hashTypes, IProgress<ulong> progress, CancellationToken ct)
		{
			...
			foreach (HashType h in hashTypes)
			{
				dict[h] = hex;
			}
