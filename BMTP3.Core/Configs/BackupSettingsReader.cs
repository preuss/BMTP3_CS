	...
	public class BackupSettingsReader {
		...
		public ConfigModel LoadDeviceConfigModel(FileInfo configFileInfo) {
			...
			try {
				...
				if(false == Toml.TryToModel(File.ReadAllText(configFileInfo.FullName), out deviceConfigModel, out diagnostics, null, modelOptions)) {
					if(diagnostics == null) {
				...
				throw new BackupConfigFileNotFoundException($"Kunne ikke finde konfigurationsfilen '{configFileInfo.FullName}'", e);
			}
