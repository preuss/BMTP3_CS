	...
	public static bool IsIncluded(string path, IEnumerable<string>? includePatterns, IEnumerable<string>? excludePatterns)
	{
		...
		if (excludePatterns != null)
		{
			foreach (string pattern in excludePatterns)
			{
				...
				if (Matches(normPath, pattern)) return false;
			}
		}
		...
		if (includePatterns != null)
		{
			...
			foreach (string pattern in includePatterns)
			{
				if (string.IsNullOrWhiteSpace(pattern)) continue;
				hasAny = true;
				if (Matches(normPath, pattern)) return true;
			}
			if (hasAny) return false; // had patterns but none matched
		}
	...
	public static string GlobToRegex(string globPattern)
	{
		...
		if (globPattern.StartsWith("@(") && globPattern.Contains(')'))
		{
			...
			if (end > 2)
			{
				...
				string remRegex = ConvertCore(remainder, ignoreAnchors: true);
		...
		if (globPattern.StartsWith("!(") && globPattern.EndsWith(')'))
		{
			...
			string positiveRegex = ConvertCore(positivePattern, ignoreAnchors: true);
		...
		if (globPattern.Contains("!(") && globPattern.EndsWith(')'))
		{
			...
			if (negStart > 0 && negEnd == globPattern.Length - 1)
			{
				...
				string prefixRegex = ConvertCore(prefixGlob, ignoreAnchors: true);
				return $"^{prefixRegex}(?!{negRegex}$)(.*)$";
			}
		}

		return ConvertCore(globPattern, ignoreAnchors: false);
	}
