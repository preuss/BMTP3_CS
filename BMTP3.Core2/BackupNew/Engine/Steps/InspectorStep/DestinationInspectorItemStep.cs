...
public class DestinationInspectorItemStep : IBackupItemStep<BackupPlan, object>
{
	...
	public DestinationInspectorItemStep(IDestinationInspector inspector, IHashGenerator hashGenerator, BackupPlan plan,
		ILogger<DestinationInspectorItemStep> logger)
	{
	...
	public async Task<object> ExecuteAsync(IBackupItem item, IProgress<ulong> progress, CancellationToken ct)
	{
		...
		try
		{
			...
			List<HashType> algorithms = Context?.HashTypes != null && Context.HashTypes.Count > 0
				? Context.HashTypes.ToList()
				: new List<HashType> { HashType.SHA2_256 };

			Dictionary<HashType, string> found = new();

			// 1) Try to read from destination sidecar
			foreach (HashType algo in algorithms)
			{
			...
			List<HashType> missing = algorithms.Where(a => !found.ContainsKey(a)).ToList();
			if (missing.Count > 0)
			{
				foreach (HashType algo in missing)
				{
					try
					{
					...
					{
						_logger.LogWarning(ex, "IDestinationInspector failed for algorithm {Algo} on {DestPath}", algo,
							destPath);
					}
				}
			}
