# Task 046: create servicecollectionextensions

**Filnavn:** 046_[NEED]_create-servicecollectionextensions.md  
**Prioritet:** [NEED]  
**Milestone-spor:** Milestone 1: Filesystem baseline via CLI

## Formaal
`DependencyInjection/ServiceCollectionExtensions.cs` — `AddBMTP3Core4()`. Tilføj NuGet-ref `Microsoft.Extensions.DependencyInjection.Abstractions`.
...
```mermaid
flowchart TD
    Services[ServiceCollection] --> Factory[BackupEngineFactory]
    Factory --> Engine[IBackupEngine]
    Engine --> Command[Console command]
```
## Klasse-/flow-note
