...
public class BackupScannerTests
{
    ...
    public async Task ScanAsync_FileSystem_YieldsAllFilesInDirectory()
    {
        ...
        try
        {
            ...
            BackupPlan plan = FilesystemPlan(tempDir, recursive: false);

            var items = new List<IBackupItem>();
            ...
            var scannedNames = items.Select(i => Path.GetFileName(i.SourcePath)).OrderBy(x => x).ToArray();
    ...
    public async Task ScanAsync_WithIncludeGlob_FiltersToMatchingFiles()
    {
        ...
        try
        {
            ...
            IBackupScanner scanner = BuildScanner();
            var plan = FilesystemPlan(tempDir, recursive: false);
    ...
    public async Task ScanAsync_WithRecursive_IncludesSubdirectoryFiles()
    {
        ...
        try
        {
            ...
            IBackupScanner scanner = BuildScanner();
            BackupPlan plan = FilesystemPlan(tempDir, recursive: true);
            ...
            var names = items.Select(i => Path.GetFileName(i.SourcePath)).OrderBy(x => x).ToArray();
