...
public class HashItemStepTests
{
    ...
    {
        var logger = new NullLogger<HashItemStep>();
    ...
    public async Task ExecuteAsync_ComputesHashes_WhenItemHasNone()
    {
        ...
        try
        {
            ...
            var context = new HashStepContext { HashTypes = new[] { HashType.SHA2_256 }, ForceRecompute = false };
            var step = BuildStep(context, new Sha256ItemHasher());
            ...
            var stored = item.Metadata.Get<Dictionary<HashType, string>>(MetadataKey.Hashes);
    ...
    public async Task ExecuteAsync_SkipsRecompute_WhenHashesAlreadyPresent_AndForceRecomputeFalse()
    {
        ...
        try
        {
            ...
            var presetHashes = new Dictionary<HashType, string> { { HashType.SHA2_256, presetHash } };
            item.Metadata.Set(MetadataKey.Hashes, presetHashes);

            // Track invocations – a real hasher that counts calls
            int callCount = 0;
            var countingHasher = new CountingHasher(onCompute: () => callCount++);

            var context = new HashStepContext { HashTypes = new[] { HashType.SHA2_256 }, ForceRecompute = false };
            var step = BuildStep(context, countingHasher);
    ...
    public async Task ExecuteAsync_Recomputes_WhenForceRecomputeTrue()
    {
        ...
        try
        {
            ...
            item.Metadata.Set(MetadataKey.Hashes, new Dictionary<HashType, string> { { HashType.SHA2_256, bogusHash } });

            var context = new HashStepContext { HashTypes = new[] { HashType.SHA2_256 }, ForceRecompute = true };
            var step = BuildStep(context, new Sha256ItemHasher());
    ...
    public async Task ExecuteAsync_SetsItemFailed_AndRethrows_WhenHasherThrows()
    {
        ...
        try
        {
            ...
            var context = new HashStepContext { HashTypes = new[] { HashType.SHA2_256 }, ForceRecompute = false };
            var step = BuildStep(context, new ThrowingItemHasher());
    ...
    private class CountingHasher : IItemHasher
    {
        ...
        public CountingHasher(Action onCompute) { _onCompute = onCompute; }
