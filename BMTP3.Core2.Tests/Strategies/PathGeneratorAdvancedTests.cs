...
public class PathGeneratorAdvancedTests
{
	...
	private static BackupItem CreateItem(string fileName, string? relativePath = null)
	{
		...
		BackupItem item = BackupItem.Create(content, fileName, relativePath);
		return item;
	}
	...
	public void GenerateRelativePath_PreserveSourceTree_WithRelativePath_PreservesFolderHierarchy()
	{
		...
		BackupPlan plan = new() { OutputStrategy = OutputStructureStrategy.PreserveSourceTree };

		BackupItem item = CreateItem("photo.jpg", @"DCIM\Camera");
	...
	public void GenerateRelativePath_PreserveSourceTree_WhenRelativePathAbsent_ReturnsFileNameOnly()
	{
		...
		BackupItem item = CreateItem("photo.jpg");
	...
	{
		PathGenerator pg = new();
		BackupItem item = CreateItem("img001.jpg");
		DateTime authored = new(2023, 7, 14, 10, 30, 0, DateTimeKind.Utc);
	...
	{
		PathGenerator pg = new();
		BackupItem item = CreateItem("img002.jpg");
		DateTime authored = new(2023, 3, 5, 0, 0, 0, DateTimeKind.Utc);
		item.Metadata.Set(MetadataKey.AuthoredDateTime, authored);
	...
	{
		PathGenerator pg = new();
		BackupItem item = CreateItem("img003.jpg");
		DateTime authored = new(2023, 3, 9, 0, 0, 0, DateTimeKind.Utc);
		item.Metadata.Set(MetadataKey.AuthoredDateTime, authored);
	...
	{
		PathGenerator pg = new();
		BackupItem item = CreateItem("myphoto.jpg");
		DateTime authored = new(2024, 1, 1, 0, 0, 0, DateTimeKind.Utc);
		item.Metadata.Set(MetadataKey.AuthoredDateTime, authored);
	...
	{
		PathGenerator pg = new();
		BackupItem item = CreateItem("img.jpg");

		DateTime authored = new(2021, 11, 25, 0, 0, 0, DateTimeKind.Utc);
		DateTime created = new(2022, 5, 10, 0, 0, 0, DateTimeKind.Utc);
	...
	public void ApplyPattern_WhenAuthoredDateTimeAbsent_FallsBackToCreatedDateTime()
	{
		...
		DateTime created = new(2020, 6, 15, 0, 0, 0, DateTimeKind.Utc);
		item.Metadata.Set(MetadataKey.CreatedDateTime, created);
	...
	public void ApplyPattern_WhenAuthoredAndCreatedAbsent_FallsBackToModifiedDateTime()
	{
		...
		DateTime modified = new(2019, 4, 22, 0, 0, 0, DateTimeKind.Utc);
