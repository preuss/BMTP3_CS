...
Number tokens are **single-character tokens**. They can be repeated to indicate the
number of digit positions. This is in contrast to date/time tokens (e.g., `YYYY`, `MM`)
which are distinct multi-character tokens with specific meanings.

| Token | Description                                                              |
|-------|--------------------------------------------------------------------------|
| `#`   | Digit (omit if zero). Repeat to indicate number of optional positions.   |
| `0`   | Digit (show if zero). Repeat to indicate number of required positions.   |
| `,`   | Thousands separator (position indicates grouping)                        |
| `.`   | Decimal separator (only one allowed)                                     |
| `;`   | Positive;negative section separator                                      |
...
Date tokens are **multi-character tokens** with distinct meanings per length.
`YYYY` is a single token meaning "4-digit year", not four `Y` tokens.
...
Time tokens are **multi-character tokens** with distinct meanings per length.
...
Eval expressions select a text segment based on the variable's value. They provide
conditional and selection logic within a message.

The eval separator is `§` (section sign, Unicode U+00A7) or `¶` (pilcrow sign,
Unicode U+00B6). Both are equivalent and interchangeable — dual separator support
...
- If no rule matches, the **last entry** is always selected as fallback —
    regardless of whether it is named `other` or not.
- `other` is a naming convention for readability, not a reserved keyword.

### 10.2. EvalType: `if` (Conditional)

Selects between two strings based on a numeric condition. Works like a ternary
...
- If `condition` is false, `falseValue` is output (last entry = fallback).
- Both `trueValue` and `falseValue` can contain nested placeholders.

#### Conditions

All conditions are numeric. The operator and operand are written as one token
(no space).

| Condition    | Description                    | Example              |
|--------------|--------------------------------|----------------------|
| `eq`N        | Equal to N                     | `eq0`, `eq100`       |
...
CLDR plural categories are resolved based on the locale the MessageFormatter was
instantiated with. If no locale was specified, the system locale is used.
...
- The **last entry** is always fallback if no rule matches — using `other` as the
    last entry is recommended for readability.
- Exact numbers are checked before CLDR categories.
- `value` can contain nested placeholders.
- The separator between rules is `|` (pipe).
- The separator between countRule and value is `#`.
- For escaping `|`, `#`, or other special characters in values, see the
    Escaping chapter.
...
```
    ...
    ${items § plural, 0 # no items | one # ${items} item | other # ${items} items}
        ...
        → "1 item"     (when items=1, CLDR 'one' matches)
        → "5 items"    (when items=5)
```

### 10.4. EvalType: `select` (Category Selection)

Selects a string based on a categorical match. Requires a **string** value.
...
- `category` is matched directly against the variable's string value.
- Matching is **case-sensitive**.
- Rules are evaluated in order; first match wins.
- The **last entry** is always fallback if no category matches — using `other` as
    the last entry is recommended for readability.
- `value` can contain nested placeholders.
- For escaping `|`, `#`, or other special characters in values, see the
    Escaping chapter.
...
```
    ${gender § select, male # he | female # she | other # they}
        ...
        → "they"  (when gender="unknown")

    ${status § select, active # user is active | inactive # user is inactive | other # unknown status}
        → "user is active"  (when status="active")
        → "unknown status"  (when status="pending")

    ${role § select, admin # Administrator: ${userName} | other # User: ${userName}}
        → "Administrator: John"  (when role="admin", userName="John")
        → "User: John"           (when role="viewer", userName="John")
```

### 10.5. Error: Unknown EvalType

If the EvalType is not recognized, a runtime error occurs.

```
    ${count § unknown, ...}    → Error: EvalType 'unknown' is not recognized
```

### 10.6. Error: Type Mismatch

If an EvalType is used with an incompatible data type, a runtime error occurs.

```
    ${name § if, eq0 ? yes : no}               → Error: 'if' requires a numeric value
    ${name § plural, 0 # zero | 1 # one}       → Error: 'plural' requires a numeric value
    ${count § select, 1 # one | 2 # two}       → Error: 'select' requires a string value
```
...
Functions can be combined with FormatType/FormatStyle, CustomPattern, or Eval
expressions.

Format expressions (FormatType/FormatStyle/CustomPattern) and Eval expressions
are **mutually exclusive** — a placeholder uses one or the other, never both.
...
Literal braces in CustomPattern content and EvalPattern output strings are escaped
by doubling.
...
Other characters (`:`, `,`, `/`, etc.) do not require escaping — they are handled
by context-aware parsing.

**Examples:**

```
    ${arg, datetime : YYYY}}MM}              → "2025}04"
    ${arg, datetime : {{filename}}}}         → "{filename}"
    ${count § if, eq0 ? no {{files}} : has files}  → "no {files}" (when count=0)
```

### 12.2. Nested Placeholders

CustomPattern content and EvalPattern output strings can contain nested
placeholders (`${...}` or `#{...}`). The parser tracks balanced braces and
does not terminate the outer pattern at a `}` that belongs to a nested
placeholder.

**Examples:**

```
    ${created, datetime : YYYY-MM-DD ${filename}}
        → "2025-04-17 photo.jpg"

    ${count § plural, 0 # no files in ${folder} | 1 # one file in ${folder} | other # ${count} files in ${folder}}
        → "5 files in Documents"  (when count=5, folder="Documents")
```

The parser distinguishes nested placeholders from literal text by recognizing
the `${` or `#{` opening sequence. To produce a literal `${`, use `${{` (the
`{{` escape produces `{`).

### 12.3. In Function Arguments

Function arguments are delimited by `(` and `)`, with `,` separating multiple
arguments. The following characters must be escaped with backslash when used
as literal values in arguments:

| Literal | Escape | Reason                          |
|---------|--------|---------------------------------|
| `)`     | `\)`   | Would close the function call   |
| `,`     | `\,`   | Would separate arguments        |
| `(`     | `\(`   | Would open nested parenthesis   |
| `{`     | `\{`   | Would start a placeholder       |
| `}`     | `\}`   | Would close a placeholder       |
| `\`     | `\\`   | Would start an escape sequence  |

All other characters (letters, digits, spaces, `/`, `:`, `-`, `.`, etc.) do not
require escaping.

**Examples:**

```
    ${arg.replace(\},x)}           → arg.replace("}", "x")
    ${arg.replace(\,,;)}           → arg.replace(",", ";")
    ${arg.replace(\(,[)}           → arg.replace("(", "[")
    ${arg.replace(hello,world)}    → arg.replace("hello", "world")
    ${arg.replace(\\,/)}           → arg.replace("\", "/")
    ${arg.padLeft(5,\))}           → arg.padLeft(5, ")")
```

### 12.4. In FormatType and FormatStyle
...
| Unbalanced `)` in function arguments                  | "Unbalanced ')' in function arguments"                                        |
...
| CustomPattern token not supported by FormatType       | "Token 'X' is not supported for FormatType 'Y'"                               |
| FormatStyle and CustomPattern both specified          | "FormatStyle and CustomPattern cannot be used together"                       |
| Eval and Format both specified                        | "Eval expression and Format expression cannot be used together"               |
| Unknown EvalType                                      | "EvalType 'X' is not recognized"                                              |
| EvalType type mismatch                                | "'if'/'plural' requires a numeric value" / "'select' requires a string value" |
| No matching rule (configurable)                       | "No matching rule for value 'X' — no fallback selected"                       |
| Invalid function arguments                            | "Invalid arguments for function 'X': Y"                                       |

The **no matching rule** behavior is configurable:
- **Strict mode:** A runtime error is raised if no rule matches.
- **Fallback mode:** The last entry is selected as fallback (default behavior).
...
```
    ${created, datetime : YYYY-MM-DD ${filename}}      → "2025-04-17 photo.jpg"
    ${count § plural, 0 # no files in ${folder} | other # ${count} files in ${folder}}
        → "no files in Documents"  (when count=0, folder="Documents")
```
...
Rules:
- FormatStyle and CustomPattern are mutually exclusive.
- CustomPattern requires FormatType.
- FormatType without FormatStyle or CustomPattern performs type assertion only.
...
Rules:
- EvalTypes: `if` (numeric), `plural` (numeric), `select` (string)
- Eval separator: `§` (U+00A7) or `¶` (U+00B6) — both are equivalent
    and interchangeable. Dual separator support is required.
- Format and Eval are mutually exclusive.

### EvalPattern Syntax

```
    if:       condition ? trueValue : falseValue
    plural:   countRule # value | countRule # value | ...
...
```
    Variable → Functions → Format Expression → String Output
                                ├─ FormatType Assertion
                                ├─ FormatStyle
                                └─ CustomPattern

    Variable → Functions → Eval Expression → String Output
                                ├─ if (numeric condition)
                                ├─ plural (numeric count rules)
                                └─ select (string category match)
```
