...
namespace BMTP3.Core2.Tests.Resilience
{
    public class ExponentialBackoffRetryPolicyTests
    {
        ...
        {
            var policy = new ExponentialBackoffRetryPolicy(maxAttempts: 3, baseBackoffMs: 0);
            int callCount = 0;

            int result = await policy.ExecuteAsync<int>(() =>
            {
                callCount++;
        ...
        public async Task ExecuteAsync_FailsTwiceThenSucceeds_ReturnsResultOnThirdAttempt()
        {
            ...
            string result = await policy.ExecuteAsync<string>(() =>
            {
                callCount++;
                if (callCount < 3)
        ...
        public async Task ExecuteAsync_CancellationRequested_ThrowsOperationCanceledException()
        {
            ...
            var policy = new ExponentialBackoffRetryPolicy(maxAttempts: 5, baseBackoffMs: 500);
        ...
        public async Task ExecuteAsync_ExponentialBackoff_TotalElapsedGrowsWithRetries()
        {
            ...
            var policy = new ExponentialBackoffRetryPolicy(maxAttempts: 3, baseBackoffMs: baseMs);
            int callCount = 0;
        ...
        public void Constructor_MaxAttemptsLessThanOne_ThrowsArgumentOutOfRangeException()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() =>
                new ExponentialBackoffRetryPolicy(maxAttempts: 0, baseBackoffMs: 100));
        }
        ...
        {
            Assert.Throws<ArgumentOutOfRangeException>(() =>
                new ExponentialBackoffRetryPolicy(maxAttempts: 3, baseBackoffMs: -1));
        }
    }
}
