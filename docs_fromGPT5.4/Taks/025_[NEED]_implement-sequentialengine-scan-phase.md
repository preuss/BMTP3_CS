# Task 025: implement sequentialengine scan phase

**Filnavn:** 025_[NEED]_implement-sequentialengine-scan-phase.md  
**Prioritet:** [NEED]  
**Milestone-spor:** Milestone 1: Filesystem baseline via CLI

## Formaal
Udvid `SequentialBackupEngine` med `ProgressTracker` i scan-fasen. Tilføj `IFileTransfer` og `ISidecarGenerator` til konstruktør. Scan-loopet kalder `progressTracker.ItemDiscovered(item)` og `progress?.Report(progressTracker.GetSnapshot())`.
...
```mermaid
flowchart LR
    Scan[Scan source] --> Resolve[Resolve destination]
    Resolve --> Transfer[Transfer file]
    Transfer --> Sidecar[Write sidecar]
    Sidecar --> Result[Update result/progress]
```
## Klasse-/flow-note
