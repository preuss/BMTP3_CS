...
public class ArchiveStorageStep : IBackupStep
{
	...
	private void GenerateSidecar(IBackupItem item, string targetFilePath, SidecarFormat format)
	{
		var data = new
		{
			...
			OriginalPath = item.Metadata.Get<string>(MetadataKey.OriginalFileName), // TODO: Note: This seems like it should be OriginalPath, not OriginalFileName
			...
			SizeBytes = item.Metadata.Get<long>(MetadataKey.Length)
		};
