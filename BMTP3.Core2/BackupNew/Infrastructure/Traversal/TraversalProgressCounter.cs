...
public class TraversalProgressCounter
{
	...
	public void IncrementFileCount(int incrementWith = 1)
	{
		...
		if (incrementWith < 0)
		{
			throw new ArgumentOutOfRangeException(nameof(incrementWith), "Negative increments are not allowed.");
		}
	...
	{
		ProgressSnapshotCount snapshot = GetSnapshot();
		PostEvent(FileCountChanged, snapshot.FileCount);
		PostEvent(DirectoryCountChanged, snapshot.DirectoryCount);
		PostEvent(CombinedCountChanged, snapshot);
	}
	...
	private void PostEvent<T>(EventHandler<T>? handler, T value)
	{
		if (handler != null)
		{
			...
			{
				(EventHandler<T> h, T v, TraversalProgressCounter sender) =
					((EventHandler<T>, T, TraversalProgressCounter))state!;
				h(sender, v);
			}, (handler, value, this));
		}
	}
