...
- Follow the repository `.editorconfig` and project-local `.editorconfig` files where they exist.
- In most repository folders, explicit local types are preferred over `var`; note that `BMTP3.Core` has its own local `.editorconfig`.
...
## Project-specific conventions: legacy pipeline stages

This guidance applies only when working in the older channel/pipeline-style implementations (primarily legacy Core2-style code). It is **not** a general Core4 architectural rule.

To avoid ambiguity and compile-time conflicts (for example `CS0263`), the older pipeline-based code standardizes pipeline-stage implementations as follows:
...
Note: solution builds/tests can bump `Directory.Build.props` via the repository build-number tooling.
