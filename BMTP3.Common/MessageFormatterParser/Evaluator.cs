...
namespace BMTP3.Common.MessageFormatterParser
{
	public class Evaluator
	{
		...
		private string EvaluatePlaceholder(PlaceholderNode node)
		{
			...
			{
				var patternResult = new StringBuilder();
					...
					if(item is TextNode text)
					{
						...
						if(node.NameOrIndex == "date" && isDate)
						{
							...
							switch(text.Value)
							{
								...
								case "EEE": patternResult.Append(date.ToString("ddd", CultureInfo.InvariantCulture).Substring(0, 3)); break;
		...
		private bool EvaluateCondition(IfConditionNode condition, object value)
		{
			...
			{
				var p = condition.ConditionParameters[0];
				param = EvaluateArg(p);
			}
		...
		{
			var func = funcCall.Name;
			// Helper to get first argument as string
			string? Arg0() => funcCall.Arguments != null && funcCall.Arguments.Count > 0 ? EvaluateArg(funcCall.Arguments[0]) : null;
			...
			if(func == "format")
			{
				...
				try
				{
					...
					if(fmt.Contains("{") && fmt.Contains("0:") && fmt.TrimEnd().EndsWith("}"))
					{
						...
						{
							var inner = fmt.Substring(colon + 1, last - colon - 1);
		...
		{
			if(node is TextNode t) return t.Value;
			if(node is LiteralNode l) return l.Value;
			if(node is PlaceholderNode p) return EvaluatePlaceholder(p);
			return null;
		}
	}
}
