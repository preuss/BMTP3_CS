...
public class TimestampFormatter : ITimestampFormatter
{
	...
	public string Format(TimestampCandidate candidate, TimestampFormatDescriptor descriptor)
	{
		...
		string? datePart = FormatDate(candidate.Date, descriptor.DateYearMonthSeparator, descriptor.DateMonthDaySeparator);
		string? timePart = FormatTime(candidate.Time, candidate.SubSeconds, descriptor.TimeFormat, descriptor.TimeClockSeparator, descriptor.TimeClockFormatStyle, descriptor.TimeFractionSeparator);
		string? offsetPart = FormatOffset(candidate.Offset, descriptor.OffsetFormat, descriptor.OffsetClockSeparator, descriptor.OffsetClockTrimStyle, descriptor.OffsetFractionSeparator);

		if(!string.IsNullOrWhiteSpace(datePart)) outputParts.Add(datePart);
		if(!string.IsNullOrEmpty(datePart) && !string.IsNullOrWhiteSpace(timePart)) outputParts.Add(Separator(descriptor.DateTimeSeparatorStyle));
		if(!string.IsNullOrEmpty(timePart)) outputParts.Add(timePart);
		if(!string.IsNullOrEmpty(offsetPart)) outputParts.Add(offsetPart);

		return string.Join(string.Empty, outputParts);
	}

	private string? FormatDate(DateOnly? dateOnly, DateComponentSeparator sepYearMonth, DateComponentSeparator sepMonthDay)
	{
		if(dateOnly is not DateOnly date) return null;
	...
	private string? FormatTime(TimeOnly? timeOnly, long? subSeconds, TimeRepresentation timeRepresentation, TimeClockSeparator? clockSep, TimeClockFormatStyle? clockTrimStyle, DecimalFractionSeparator fractionSeparator)
	{
		...
		if(timeRepresentation == TimeRepresentation.Clock)
		{
			if(clockSep == null)
			{
				throw new ArgumentOutOfRangeException(nameof(clockSep), clockSep, $"And {nameof(timeRepresentation)} of type {TimeRepresentation.Clock} needs a {nameof(clockSep)}");
			}
			if(clockTrimStyle == null)
			{
				throw new ArgumentOutOfRangeException(nameof(clockTrimStyle), clockTrimStyle, $"And {nameof(timeRepresentation)} of type {TimeRepresentation.Clock} needs a {nameof(clockTrimStyle)}");
			}
	...
	private string? FormatTimeAsClock(TimeOnly? timeOnly, long? subSeconds, TimeClockSeparator clockSep, TimeClockFormatStyle clockTrimStyle, DecimalFractionSeparator fractionSeparator)
	{
		...
		if(subSeconds < 0)
		{
			throw new ArgumentOutOfRangeException(nameof(subSeconds), subSeconds, "SubSeconds must be null or minimum 0 and larger.");
		}
		...
		return FormatTimeValuesAsClock(hours, minutes, seconds, subSeconds, clockSep, clockTrimStyle, fractionSeparator);
	}

	private string FormatTimeValuesAsClock(int hours, int minutes, int seconds, long? fractions, TimeClockSeparator clockSep, TimeClockFormatStyle clockTrimStyle, DecimalFractionSeparator fractionSeparator)
	{
	...
	private string? FormatOffset(TimeSpan? offsetSpan, TimeRepresentation timeOffsetRepresentation, TimeClockSeparator? clockSep, TimeClockFormatStyle? clockTrimStyle, DecimalFractionSeparator fractionSeparator)
	{
		// If offset nonexisting or unknown, then empty offset.
		if(offsetSpan is not TimeSpan offset) return string.Empty;
		...
		if(timeOffsetRepresentation == TimeRepresentation.Clock)
		{
			if(clockSep == null)
			{
				throw new ArgumentOutOfRangeException(nameof(clockSep), clockSep, $"And {nameof(timeOffsetRepresentation)} of type {TimeRepresentation.Clock} needs a {nameof(clockSep)}");
			}
			if(clockTrimStyle == null)
			{
				throw new ArgumentOutOfRangeException(nameof(clockTrimStyle), clockTrimStyle, $"And {nameof(timeOffsetRepresentation)} of type {TimeRepresentation.Clock} needs a {nameof(clockTrimStyle)}");
			}
			absOffsetStr = FormatOffsetAsClock(offset.Duration(), clockSep.Value, clockTrimStyle.Value, fractionSeparator);
		} else
		{
	...
	private string FormatOffsetAsClock(TimeSpan offset, TimeClockSeparator clockSep, TimeClockFormatStyle clockTrimStyle, DecimalFractionSeparator fractionSeparator)
	{
		if(offset < TimeSpan.Zero)
		{
			throw new ArgumentOutOfRangeException(nameof(offset), offset, "Offset must be non-negative (absolute) when formatting clock offset.");
		}
		if(offset == TimeSpan.Zero)
		{
			return string.Empty;
		...
		string formattedTimeStr = FormatTimeValuesAsClock(totalHours, minutes, seconds, subSeconds, clockSep, clockTrimStyle, fractionSeparator);
	...
	private string FormatOffsetAsDuration(TimeSpan offset, DecimalFractionSeparator fractionSep)
	{
		if(offset < TimeSpan.Zero)
		{
			throw new ArgumentOutOfRangeException(nameof(offset), offset, "Offset must be non-negative (absolute) when formatting duration offset.");
		}
		if(offset == TimeSpan.Zero)
		{
			return string.Empty;
		...
		string formattetDurationOffset = FormatTimeValuesAsDuration(totalHours, minutes, seconds, subSeconds, fractionSep);
	...
	private string FormatTimeValuesAsDuration(int hours, int minutes, int seconds, long? fractions, DecimalFractionSeparator sep)
	{
		// Negative time not allowed, ± sign handled outside this method
		if(hours < 0 || minutes < 0 || seconds < 0 || fractions < 0)
		{
		...
		if(includeMinutes) durationStr.Append(minutes.ToString()).Append("M");
		if(includeSeconds)
		{
			durationStr.Append(seconds.ToString());
			if(includeFraction) durationStr.Append(Separator(sep)).Append(fractionStr);
	...
	private string Separator(DateComponentSeparator sep)
	{
		return sep switch
		{
			...
			DateComponentSeparator.Hyphen_Minus => "\u002D",     // YYYY-MM-DD, U+002D, 0x2D, standard ASCII hyphen-minus, ISO 8601 default
			DateComponentSeparator.Hyphen => "\u2010",           // YYYY‐MM‐DD, U+2010, 0x2010, true hyphen
			DateComponentSeparator.Minus_Sign => "\u2212",       // YYYY−MM−DD, U+2212, 0x2212, mathematical minus sign, ISO 8601 recommended
