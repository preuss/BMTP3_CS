...
public class Lexer : ILexer
{
	...
	private static readonly HashSet<string> knownEvalTypes = new()
	{
		"eq0", "gt0", "gte0", "lt0", "lte0", "ne0", "in", "nin"
	};

	private static readonly HashSet<string> knownStyles = new()
	{
	...
	public Token NextToken()
	{
		...
		if (_state == LexerState.Base)
		{
			...
			{
				List<char> chars = _charStream.Peek(2);
				if (chars[0] == '$' && chars[1] == '{')
				{
					NextChar();
					NextChar();
		...
		else if (_state == LexerState.Placeholder)
		{
			...
			{
				NextChar();
				return new Token(TokenType.Colon, ":", startPos, startLine, startCol);
			}

			if (c == '(')
			{
				NextChar();
				return new Token(TokenType.ParenOpen, "(", startPos, startLine, startCol);
			}

			if (c == ')')
			{
				NextChar();
				return new Token(TokenType.ParenClose, ")", startPos, startLine, startCol);
			}

			if (c == '§')
			{
				NextChar();
				return new Token(TokenType.Section, "§", startPos, startLine, startCol);
			}

			if (c == '?')
			{
				NextChar();
				return new Token(TokenType.QuestionMark, "?", startPos, startLine, startCol);
			}
