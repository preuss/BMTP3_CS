...
Filsystem-attributter skal udfyldes i metadata-readerens output (`ExtractedMetadata`) og må ikke implementeres som en skjult, separat engine-fallback uden om metadata-laget.
...
2. Keepalive mindst hver 30. sekund under lange scan/transfer-forløb.
3. Operation timeout pr. read/copy: 60 sekunder som standard.
4. Transiente device-fejl retry'es med 3 forsøg (1s, 2s, 4s); permanent disconnect behandles ikke som transient.
...
**Minimum i nuværende skeleton-contract:**
...
**Udvidet rapportering (senere tier / notifier-lag):**
...
Rapportering til `IProgress<IBackupProgress>` skal være debounce'et; praktisk standard er **maks én opdatering pr. 500 ms**, ikke én opdatering pr. fil.
...
- Logging + progress debounce
- Tydelig error policy per fase
- Tydelig gating af endnu ikke implementerede feature-flags
...
- `IBackupRepository` / persistence-lag
- Resume state og idempotent genkørsel
- Sikker skip/retry af allerede behandlede items

**Exit-kriterie:** resume virker end-to-end uden implicit state.

## Tier 6 (advanced UI / reporting)

- `IProgressNotifier`
- Rich CLI/UI events
- ETA / throughput / active workers som udvidet rapportering

**Exit-kriterie:** UI-lag giver bedre feedback uden at ændre backup-korrekthed.
