    ...
    public class SidecarReaderTests
    {
        ...
        public async Task Reads_Hashes_From_Json_Hashes_Object()
        {
            ...
            try
            {
                ...
                var val = await SidecarReader.TryReadHashFromSidecarAsync(file, HashType.SHA2_256, CancellationToken.None);
        ...
        public async Task Reads_Hashes_From_Json_With_UTF8_BOM_And_Falls_Back_On_Malformed_JSON_To_Ini()
        {
            ...
            try
            {
                ...
                byte[] malformed = System.Text.Encoding.UTF8.GetBytes("{ \"hashes\": { \"SHA2_256\": \"dead\"");
                using(var fs = File.OpenWrite(sidecar))
                {
            ...
            finally
            {
                try { File.Delete(file); } catch { }
                try { File.Delete(sidecar); } catch { }
                try { File.Delete(Path.ChangeExtension(file, ".meta")); } catch { }
                try { Directory.Delete(dir); } catch { }
            }
        }
        ...
        public async Task Reads_Hashes_From_Json_DirectRoot_Key_And_Ini_Casing()
        {
            ...
            try
            {
                ...
                var valJson = await SidecarReader.TryReadHashFromSidecarAsync(file, HashType.SHA2_256, CancellationToken.None);
                ...
                var valIni = await SidecarReader.TryReadHashFromSidecarAsync(file, HashType.SHA2_256, CancellationToken.None);
                Assert.Equal("feedfaceini", valIni);
            }
            finally
            {
                try { File.Delete(file); } catch { }
                try { File.Delete(sidecarJson); } catch { }
                try { File.Delete(sidecarIni); } catch { }
                try { Directory.Delete(dir); } catch { }
            }
        }
    }
