...
public sealed class ItemStateMachine
{
		...
		{
			{
				...
				new[] { ItemResultState.Success, ItemResultState.Failed, ItemResultState.Skipped }
			},
	...
	{
		return LifecycleTransitions.TryGetValue(from, out ItemLifecycleState[]? allowed) && Contains(allowed, to);
	}
	...
	{
		return ResultTransitions.TryGetValue(from, out ItemResultState[]? allowed) && Contains(allowed, to);
	}
