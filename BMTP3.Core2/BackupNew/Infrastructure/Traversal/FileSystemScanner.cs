...
public sealed class FileSystemScanner : ITraversalScanner<FileInfo>
{
	...
	/// </summary>
	public IEnumerable<FileInfo> Scan(string rootPath, bool recursive = true,
	...
	/// </summary>
	public async IAsyncEnumerable<FileInfo> ScanAsync(string rootPath, bool recursive = true,
		...
		[EnumeratorCancellation] CancellationToken cancellationToken = default)
	{
		...
		await foreach (FileInfo file in ScanInternalAsync(
			               dirInfo, recursive,
			               f =>
			               {
	...
	private async IAsyncEnumerable<FileInfo> ScanInternalAsync(DirectoryInfo dir, bool recursive,
		Action<FileInfo> onFile, Action<DirectoryInfo> onDirectory,
		[EnumeratorCancellation] CancellationToken cancellationToken)
	{
		...
		if (recursive)
		{
			foreach (DirectoryInfo subDir in SafeGetDirectories(dir))
			{
				...
				await foreach (FileInfo f in ScanInternalAsync(subDir, recursive, onFile, onDirectory,
					               cancellationToken))
				{
					yield return f;
				}
